//@From TinyMCE - GNU LESSER GENERAL PUBLIC LICENSE
	...
	var xtc = 'xenMCE.Templates.ColorPicker';
	xenMCE.Templates.ColorPicker = 
	{
		...
		showColor: function(name, hexa) {
			if (name)
				$('#picker_colorname').html(name);
		
			$('#picker_preview').css('backgroundColor', hexa);
			$('#picker_color').val(hexa);
		},
		...
		generatePicker: function () {
			...
			$('.picker_gs').each(function(index, value) {
		...
		generateNamedColors: function()  {
			var t = xenMCE.Templates.ColorPicker;
			var $el = $('#picker_namedcolors'), h = '', n, v, i = 0;
		
			if ($el.hasClass('generated'))
				return;

			$.each(t.named, function(hexa, name){
				$('<a style="background-color: ' + hexa + '" />')
				.click(function(){
					t.changeFinalColor(name, hexa);
					t.showColor(name, hexa);
		...
		changeFinalColor: function(color) {
			...
			this.setCol($('#picker_preview'), color);
			$('#picker_color').val(color);
		},
